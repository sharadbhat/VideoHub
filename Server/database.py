...
class Database:
    def __init__(self): # WORKS
        self.db = pymysql.connect(host="127.0.0.1", user="root", passwd="sdhc3189", db="video",port=3306)
    ...
    def get_most_viewed(self): # WORKS
        ...
        """
#         with sqlite3.connect("lala_video_web.db") as con:
#             cur = con.cursor()
        self.cur.execute("SELECT video_id FROM youtube_video ORDER BY CAST(viewCount as decimal) DESC LIMIT 10")
    ...
    def get_video_title(self, video_ID): #WORKS
        ...
        """
        try:
            self.cur.execute("SELECT video_title FROM youtube_video WHERE video_ID = \"{}\"".format(video_ID))
    ...
    def update_view_count(self, video_ID): #WORKS
        ...
        """
        try:
            self.cur.execute("UPDATE youtube_videos SET viewCount = viewCount + 1 WHERE video_ID = \"{}\"".format(video_ID)) # Adds 1 to the existing value.
            self.db.commit()
        except:
            self.db.rollback()
    ...
    def get_video_uploader(self, video_ID): #WORKS
        ...
        """
        try:
            done = self.cur.execute("SELECT channel_title FROM youtube_video WHERE video_id = \"{}\"".format(video_ID))
    ...
    def get_upload_date(self, video_ID):
        ...
        """
        self.cur.execute("SELECT collection_date FROM youtube_video WHERE video_id = \"{}\"".format(video_ID))
        self.db.commit()
    ...
    def get_random_ID(self): # WORKS
        ...
        """
        self.cur.execute("SELECT video_id FROM youtube_video ORDER BY RAND() LIMIT 1") # Selects video_ID from 1 random row.
        self.db.commit()
        return self.cur.fetchone()[0]
    ...
    def get_views(self, video_ID): # WORKS
        ...
        """
        self.cur.execute("SELECT viewCount FROM youtube_video WHERE video_id = \"{}\"".format(video_ID))
        self.db.commit()
        return self.cur.fetchone()[0]
    ...
    def is_available(self, video_ID): # WORKS
        ...
        """
        done = self.cur.execute("SELECT video_id FROM youtube_video WHERE video_id = \"{}\"".format(video_ID))
        self.db.commit()
        if done == 1:
            return True
        else:
    ...
    def get_uploaded(self, username): # WORKS
        ...
        """
        self.cur.execute("SELECT video_id FROM youtube_video WHERE channel_title = \"{}\"".format(username))
        uploaded_video_IDs = []
        for ID in self.cur.fetchall():
    ...
    def get_five_random_IDs(self):
        ...
        """
        self.cur.execute("SELECT video_id FROM youtube_video ORDER BY RAND() LIMIT 5")
        IDs = []
        for ID in self.cur.fetchall():
            IDs.append(ID[0])
        self.db.commit()
    ...
    def get_video_num(self, username):
        ...
        """
        done = self.cur.execute("SELECT video_id FROM youtube_video where channel_title = \"{}\"".format(username))
        self.db.commit()
        return done
    ...
    def video_dict(self):
        ...
        - For Fuzzy Search.
        """
        self.cur.execute("SELECT video_id, video_title FROM youtube_video")
        videos = {}
        video_titles = []
