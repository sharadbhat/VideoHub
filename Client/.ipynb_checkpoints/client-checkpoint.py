...
def start(): #WORKS
    ...
    for ID in most_viewed_video_IDs:
        video = HTML_TEMPLATE.substitute(youtube_id=ID)
        ...
        details = [video,title, views, uploader]
...
#         vid_video = ((requests.get(url='http://127.0.0.1:8080/video/{}'.format(video_ID))).content).decode("utf-8") # Done
        vid_video = HTML_TEMPLATE.substitute(youtube_id=video_ID)
        ...
        for ID in random_video_IDs:
            video = HTML_TEMPLATE.substitute(youtube_id=ID)
#             video = ((requests.get(url='http://127.0.0.1:8080/video/{}'.format(ID))).content).decode("utf-8") # Done
            title = ((requests.get(url='http://127.0.0.1:8080/title/{}'.format(ID))).content).decode("utf-8") # Done
            views = ((requests.get(url='http://127.0.0.1:8080/views/{}'.format(ID))).content).decode("utf-8") # Done
            uploader = ((requests.get(url='http://127.0.0.1:8080/uploader/{}'.format(ID))).content).decode("utf-8") # Done
            details = [video,title, views, uploader]
        ...
        if 'user' in session:
            ...
            username = session['user']
            return render_template_string(video_page, random_vids = random_vids, video_ID = video_ID, video=vid_video,title = vid_title, uploader = vid_uploader, views = vid_views, vid_upload_date = vid_upload_date, logged_in = True, username = username)
        else:
...
def watched_videos():
    ...
    if request.method == 'GET':
        ...
        for ID in watched_IDs:
            ...
            views = ((requests.get(url='http://127.0.0.1:8080/views/{}'.format(ID))).content).decode("utf-8") # Done
            uploader = ((requests.get(url='http://127.0.0.1:8080/uploader/{}'.format(ID))).content).decode("utf-8") # Done
            watched_dictionary.update({ID : [video,title, views, uploader]})
